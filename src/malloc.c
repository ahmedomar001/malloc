...
struct _block * last_allocation = NULL; /* Pointer to the last _block allocated */
...
// \TODO Put your Next Fit code in this #ifdef block
   ...
   struct _block *curr = last_allocation;

   // Start from the last allocation and iterate until we reach the end
   while (curr) {
      if (curr->free && curr->size >= size) {
         // Found a suitable free block, return it
         return curr;
      }
   ...
   // If we reach here, we couldn't find a suitable block, so return NULL
   return NULL;
#endif
...
void *malloc(size_t size) 
{
   ...
   */
   ...
   /* Hint: Pointer to the last _block allocated
      Used for next fit start search position
   */
...
void free(void *ptr) 
{
   ...
   */
   ...
   if (curr != heapList)
   {
      prev = (struct _block *)((char *)curr - sizeof(struct _block));
      if (prev->free)
      {
...
void *calloc( size_t nmemb, size_t size )
{
 ...
 // Calculate the total size needed for the allocation
   ...
   // If allocation successful, set the memory to zero
   if (ptr != NULL)
   {
